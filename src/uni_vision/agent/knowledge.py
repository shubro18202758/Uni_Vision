...
class CameraErrorProfile:
    ...
    def top_confusions(self, n: int = 5) -> list[tuple[str, int]]:
        return sorted(self.common_confusions.items(), key=lambda x: -x[1])[:n]
...
class KnowledgeBase:
    ...
    def record_observation(self, obs: PlateObservation) -> None:
        ...
        if obs.was_corrected and obs.original_ocr_text:
            ...
            for a, b in zip(raw, corrected, strict=False):
                if a != b:
                    key = f"{a}→{b}"
                    profile.common_confusions[key] = profile.common_confusions.get(key, 0) + 1
                    self._error_patterns[obs.camera_id][key] += 1

        # Running average confidence
        n = profile.total_detections
        profile.avg_confidence = (profile.avg_confidence * (n - 1) + obs.confidence) / n
    ...
    def get_plate_frequency(self, *, top_n: int = 20) -> list[tuple[str, int]]:
        """Return the most frequently seen plates."""
        return sorted(self._plate_frequency.items(), key=lambda x: -x[1])[:top_n]

    def get_camera_profile(self, camera_id: str) -> CameraErrorProfile | None:
    ...
    def get_cross_camera_plates(self, *, min_cameras: int = 2) -> dict[str, list[str]]:
        ...
        return {plate: cams for plate, cams in plate_cameras.items() if len(cams) >= min_cameras}

    def get_error_patterns(self, camera_id: str | None = None, *, top_n: int = 10) -> dict[str, list[tuple[str, int]]]:
        ...
        if camera_id:
            ...
            return {camera_id: sorted(patterns.items(), key=lambda x: -x[1])[:top_n]}

        result: dict[str, list[tuple[str, int]]] = {}
        for cam_id, patterns in self._error_patterns.items():
            result[cam_id] = sorted(patterns.items(), key=lambda x: -x[1])[:top_n]
        return result

    def get_recent_feedback(self, *, hours_back: float = 24, limit: int = 50) -> list[FeedbackEntry]:
        """Return recent feedback entries."""
        cutoff = time.time() - (hours_back * 3600)
        recent = [f for f in self._feedback if f.timestamp >= cutoff]
        return recent[-limit:]

    def get_anomalies(self, *, hours_back: float = 1, spike_threshold: float = 3.0) -> list[dict[str, Any]]:
    ...
    def get_camera_hints(self, camera_id: str) -> str:
        ...
        hints = ["This camera commonly confuses: "]
    ...
    async def save_to_db(self, pg_client: Any) -> None:
        ...
        try:
            ...
            logger.info("kb_persisted cameras=%d", len(self._camera_profiles))
    ...
    async def load_from_db(self, pg_client: Any) -> None:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                rows = await conn.fetch("SELECT key, category, data FROM agent_knowledge")
