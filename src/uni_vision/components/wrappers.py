...
class BuiltinDetectorComponent(CVComponent):
    ...
    async def execute(
        ...
        *,
        ...
        if ComponentCapability.PLATE_DETECTION in self._meta.capabilities and isinstance(data, np.ndarray):
...
class HuggingFaceModelComponent(CVComponent):
    ...
    async def load(self, *, device: str = "cuda") -> None:
        ...
        try:
            ...
            self._set_state(ComponentState.READY)
            log.info("hf_model_loaded repo_id=%s device=%s pattern=%s", self._repo_id, device, self._load_pattern)
    ...
    def _load_via_hub(self, device: str) -> Any:
        ...
        _, model_name = self._model_class.rsplit(".", 1) if "." in self._model_class else ("", self._model_class)
    ...
    def _infer_sync(self, data: Any, context: dict[str, Any] | None) -> Any:
...
class PipPackageComponent(CVComponent):
    ...
    async def load(self, *, device: str = "cuda") -> None:
        ...
        try:
            ...
            self._set_state(ComponentState.READY)
            log.info(
                "pip_component_loaded package=%s class=%s.%s", self._package_name, self._entry_module, self._entry_class
            )
        except asyncio.TimeoutError:
            self._load_error = f"Load timed out after {self._load_timeout}s"
            self._set_state(ComponentState.FAILED)
    ...
    def _run_sync(self, data: Any, context: dict[str, Any] | None) -> Any:
        for method_name in ("predict", "process", "detect", "ocr", "__call__"):
            fn = getattr(self._instance, method_name, None)
            if callable(fn):
                return fn(data)
        raise RuntimeError(f"Package {self._package_name} instance has no usable inference method")
...
class TorchHubComponent(CVComponent):
    ...
    async def load(self, *, device: str = "cuda") -> None:
        ...
        try:
            ...
            self._set_state(ComponentState.READY)
            log.info("torchhub_model_loaded repo=%s model=%s device=%s", self._hub_repo, self._model_name, device)
        except asyncio.TimeoutError:
    ...
    async def unload(self) -> None:
        ...
        if self._model is not None:
            ...
            try:
                ...
                pass
        self._set_state(ComponentState.REGISTERED)
        log.info("torchhub_model_unloaded repo=%s model=%s", self._hub_repo, self._model_name)
    ...
    def _infer_sync(self, data: Any, context: dict[str, Any] | None) -> Any:
        if hasattr(self._model, "predict"):
            ...
            return self._model(data)
        raise RuntimeError(f"TorchHub model {self._hub_repo}:{self._model_name} has no inference method")
